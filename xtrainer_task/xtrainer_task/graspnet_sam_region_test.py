#!/usr/bin/env python3
"""GroundedSAM2 (Grounding DINO + SAM2) 白色方块区域分割 + GraspNet 组合测试。

场景假设:
    白色方块 (white square) 是一个工作区域/容器，目标物体放在白色方块
    上面或内部。测试流程:
    ...
    3. SAM2 切分出白色方块的 2D mask。
    4. 点云滤波: 删除 **SAM mask 内** 的点云 (白色方块本身)，同时删除
       **白色方块凸包 footprint 之外** 的点云 (含 DINO 识别框到 mask 之间
       的空白)，只保留方块凸包内部没有被 mask 的物品点云。
    5. 在保留的点云子集上运行 GraspNet 生成抓取姿态并输出。

显示:
    - OpenCV 窗口: SAM2 分割结果 (框 + mask) + 保留区域
    - Open3D 窗口: 保留的物品点云 + GraspNet 夹爪 (流式刷新)
...
Boot::
    ...
    ros2 run xtrainer_task graspnet_sam_region_test --ros-args \
...
class GraspNetSamRegionTestNode(Node):
    """ROS2 节点：白色方块区域分割点云 + GraspNet 抓取检测，流式显示。"""

    def __init__(self):
        super().__init__("graspnet_sam_region_test")
        ...
        # 白色方块凸包 footprint 外扩像素: 物体略微伸出方块边缘时仍保留
        self.declare_parameter("region_margin_px", -10)
        # 是否在保留掩码内再次删除 mask 像素 (False 则只按 footprint 裁切)
        self.declare_parameter("drop_mask_pixels", True)
        ...
        self._region_margin = self.get_parameter("region_margin_px").value
        self._drop_mask_pixels = self.get_parameter("drop_mask_pixels").value
    ...
    def _process_frame(self, image_msg: Image, cloud_msg: PointCloud2):
        """单帧处理：SAM2 分割白色方块 → 保留包围盒内非 mask 点云 → GraspNet。"""
        t0 = time.time()
        ...
        # ── 4. 由白色方块 mask 构建保留掩码 ────────────────────
        #    删除 mask 内 + mask 包围盒外的点, 只保留盒内未被 mask 的物品
        keep2d = self._build_region_keep_mask(det, H, W)
        ...
        if self._show_2d:
            ...
            if keep2d is not None:
                # 绿色: 保留区域 (包围盒内非 mask); 红色: mask
                overlay = annotated.copy()
                overlay[keep2d] = (0, 255, 0)
            ...
            cv2.imshow("GroundedSAM2 Region Result", annotated)
    ...
    # ── 保留掩码构建 ─────────────────────────────────────────
    def _build_region_keep_mask(self, det, H_pc, W_pc):
        """由白色方块 mask 构建 (H_pc, W_pc) 的保留掩码。

        规则:
            - region = mask 的凸包 (白色方块实际占地), 外扩 ``region_margin_px``
            - 只保留 region 内且 **不是** mask 的像素 (方块上未被 mask 的物品)
            - mask 内删除; 方块凸包之外 (含 DINO 识别框到 mask 之间) 也删除

        Returns
        -------
        np.ndarray or None
            (H_pc, W_pc) bool, True=保留对应点云; 无检测时返回 None
        """
        if len(det.boxes) == 0:
            return None
        ...
        if self._mask_index < 0:
            ...
            if idx >= len(order):
                idx = 0
            mask2d = masks[order[idx]]

        if not np.any(mask2d):
            return None

        # ── 凸包 footprint (外扩 margin 像素) ─────────────────
        # 包围盒会包含方块之外的空白 (识别框→mask 那一段), 改用凸包贴合方块轮廓
        m = int(self._region_margin)
        contours, _ = cv2.findContours(
            mask2d.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        if not contours:
            return None
        hull_pts = np.concatenate([cv2.convexHull(c).reshape(-1, 2) for c in contours])

        region = np.zeros((H_pc, W_pc), dtype=np.uint8)
        cv2.fillConvexPoly(region, hull_pts, 1)
        if m > 0:
            kernel = np.ones((m * 2 + 1, m * 2 + 1), np.uint8)
            region = cv2.dilate(region, kernel)
        region = region.astype(bool)

        if self._drop_mask_pixels:
            keep = region & ~mask2d
        else:
            keep = region
        return keep
...
def main():
    ...
    node = GraspNetSamRegionTestNode()
    ...
    vis.create_window(
        window_name="GraspNet + GroundedSAM2 Region - Segmented Point Cloud",
        width=1280,
        height=720,
    )

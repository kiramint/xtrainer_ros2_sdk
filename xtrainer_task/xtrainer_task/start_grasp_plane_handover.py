...
class XTrainerTask(Node):
    ...
    def step_place_object(self):
        ...
        plan_result = self.mover.plan_joints(self.pick_arm,pose_joint,planner=Planner.pilz_ptp)
        if plan_result is None:
            self.get_logger().error(f"################### Moveit {self.pick_arm} Planning Failed #################")
            return False
        self.mover.execute(plan_result.trajectory)
    ...
    def step_handover_rectangle(self):
        pose_left = Pose()
        pose_left.position.x = 0.5432583689689636
        pose_left.position.y = -0.1554596722126007
        pose_left.position.z = 0.692965030670166
        pose_left.orientation.x = -0.4952966868877411
        pose_left.orientation.y = 0.5046651363372803
        pose_left.orientation.z = -0.49528488516807556
        pose_left.orientation.w = 0.5046654343605042

        pose_right = Pose()
        pose_right.position.x = 0.5771988034248352
        pose_right.position.y = -0.1565919816493988
        pose_right.position.z = 0.7267522811889648
        pose_right.orientation.x = -0.4953630268573761
        pose_right.orientation.y = -0.5045972466468811
        pose_right.orientation.z = 0.49536439776420593
        pose_right.orientation.w = 0.5045901536941528
        ...
        if self.pick_arm == "Arm1":
            ...
            pre_pose_right.position.x = pre_pose_right.position.x + 0.2
            ...
            plan_result = self.mover.plan_pose("Arm2",pose_right,planner=Planner.pilz_ptp,tip_link="L2_gripper_tcp")
            if plan_result is None:
                self.get_logger().error(f"################### Moveit {self.pick_arm} Planning Failed #################")
                return False
            self.mover.execute(plan_result.trajectory)
            ...
            post_pose_left.position.x = post_pose_left.position.x - 0.2
            plan_result = self.mover.plan_pose("Arm1",post_pose_left,planner=Planner.pilz_ptp,tip_link="L1_gripper_tcp")
            if plan_result is None:
                self.get_logger().error(f"################### Moveit {self.pick_arm} Planning Failed #################")
                return False
            self.mover.execute(plan_result.trajectory)
            ...
            pre_pose_left.position.x = pre_pose_left.position.x - 0.2
            ...
            # stage 3 
            self.get_logger().info("Stage 3")
            plan_result = self.mover.plan_pose("Arm1",pose_left,planner=Planner.pilz_ptp,tip_link="L1_gripper_tcp")
            if plan_result is None:
                self.get_logger().error(f"################### Moveit {self.pick_arm} Planning Failed #################")
                return False
            self.mover.execute(plan_result.trajectory)
            ...
            post_pose_right.position.x = post_pose_right.position.x + 0.2
            plan_result = self.mover.plan_pose("Arm2",post_pose_right,planner=Planner.pilz_ptp,tip_link="L2_gripper_tcp")
            if plan_result is None:
                self.get_logger().error(f"################### Moveit {self.pick_arm} Planning Failed #################")
                return False
            self.mover.execute(plan_result.trajectory)
